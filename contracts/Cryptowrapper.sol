...
import "./crypto-bliz.sol";
import "./SafeMath.sol";

contract Cryptowrapper is crypto-bliz {
    ...
    //for participation on CRYPTO based on eth bid
    ...
    // check when Crypto CRYPTO is ended and trading has started
    ...
    constructor  (address payable _pWallet, uint _ceiling, uint _priceFactor, uint _softCap)
        crypto-bliz(_pWallet, _ceiling, _priceFactor)  public {
    ...
    function claimtokenBonus () public returns(bool success)  {
        ...
        if ((_personalHash == TokenReferrals[_personalHash].hash) 
            ...
            TokenReferrals[_personalHash].totalTokensEarned = 0;
            crypto-blizToken.transfer(TokenReferrals[_personalHash].addr , TokensToTransfer1);
    ...
    function claimCampaignTokenBonus(bytes4 _campaignHash) public returns(bool success)  {
        ...
        if ((_marketingCampaignHash == TokenReferrals[_campaignHash].hash) 
            ...
            TokenReferrals[_campaignHash].totalTokensEarned = 0;
            crypto-blizToken.transfer(TokenReferrals[_campaignHash].addr , TokensToTransfer1);
    ...
    function transferUnsoldTokens(uint _unsoldTokens, address _addr)
...

     {
        ...
        crypto-blizToken.transfer(_addr, _unsoldTokens);
    }
